...
from .utils import get_context, append_to_response
...
Decision:"""
        ...
        try:
            ...
            append_to_response([{"analyse_document_content_rag_orchestrator":response}], filename="check_LLM_cost.json")
...
JSON Plan:"""
        response = await self.llm.ainvoke(prompt)
        append_to_response([{"set_goal_agent_rag_orchestrator":response}], filename="check_LLM_cost.json")
...
Execute this task. If the task is to find information in the document, state the information clearly. If the task requires calling an API, use the available tool."""
        ...
        append_to_response([{"perform_action_rag_orchestrator":response}], filename="check_LLM_cost.json")
...
Final Answer:"""
        response = await self.llm.ainvoke(prompt)
        append_to_response([{"answer_query_rag_orchestrator":response}], filename="check_LLM_cost.json")
